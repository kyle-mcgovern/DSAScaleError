...
for(noise_val in noise_vals) {
	noise <-c(rep(0, 92), rep(noise_val, 92))
	...
	GSEA_results <- cbind(GSEA_results, gsea_parallel_matrix(noise_adj_W, X, path_inds, 2, permutation_matrix=permutations, cores=47)$p_value)
...
colnames(GSEA_results) <- noise_vals
write.table(GSEA_results, paste0("GSEA_LFC_C2_constant.txt"), quote=F, sep="\t")
row.names(camera_lfc_results) <- names(path_names)
colnames(camera_lfc_results) <- noise_vals
write.table(camera_lfc_results, paste0("CAMERA_LFC_C2_constant.txt"), quote=F, sep="\t")

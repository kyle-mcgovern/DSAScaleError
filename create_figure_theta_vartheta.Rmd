...
# Mean point
g <- g + annotate("point", x=mean(df$x), y=height, color="black", shape=8, size=1.5, stroke=1.25, family="Arial")
...
g <- g + annotate("text", x=mean(df$x)+0.11, y=height-0.3, hjust=0, label=TeX("$mean(\\theta_1$,...,$\\theta_D)$"), size=11/.pt, color="black", family="Arial")

# 0
g <- g + geom_segment(x=0, xend=0, y=height-0.1, yend=height+0.1, color="black")
g <- g + annotate("text", x=0, y=height-0.17, label=0, size=11/.pt, color="black", family="Arial")
...
g <- g + annotate("text", x=(mean(df$x)-0.6)/2, y=height+0.25+0.125, label=TeX("$\\vartheta_1=\\theta_1-mean(\\theta_1$,...,$\\theta_D)$"), hjust=0.5, size=11/.pt, color="black", family="Arial")
...
g <- g + annotate("text", x=(-0.6)/2, y=height-0.35-0.125, label=TeX("$\\theta_1$"), hjust=0.5, size=11/.pt, color="black", family="Arial")
...
g <- g + annotate("text", x=legend_offset-0.425, y=legend_offset_y+0.25, color=text_color, hjust=0, label="In Set", size=10/.pt, family="Arial")

g <- g + annotate("point", x=legend_offset+0, y=legend_offset_y+0.25, color=text_color, shape=21, size=3, stroke=1.4)
g <- g + annotate("text", x=legend_offset+0.075, y=legend_offset_y+0.25, color=text_color, hjust=0, label="Not In Set", size=10/.pt, family="Arial")
...
# Legend Box
g <- g + geom_rect(xmin=legend_offset-0.6, xmax=legend_offset+0.575, ymin=legend_offset_y+0.125, ymax=legend_offset_y+0.4, color=text_color, alpha=0, size=0.5)
...
ggsave("~/data/output/figure_3.svg", g, units="in", width=5, height=1.8)
```

...
#m <- read.table("figure_s1_data.txt")
#head(m)
...
create_fig_2_line <- function(data) {
	...
	ds.labs <- c("Ds = 15", "Ds = 50", "Ds = 150")
	names(ds.labs) <- c(15, 50, 150)
	d.labs <- c("D = 500", "D = 10000", "D = 20000")
	names(d.labs) <- c(500, 10000, 20000)
	g <- g + facet_grid(vars(D), vars(Ds), labeller=labeller(Ds=ds.labs, D=d.labs))
	g <- g + xlab(expression(epsilon))
...
ggsave("~/data/output/figure_s1.png", g6, units="in", width=9, height=8, dpi=400)

...
g1 <- g1+ xlab(TeX("Error in $\\theta^{\\perp}$ (ϵ)"))
...
g2 <- ggplot(all_df, aes(x=as.factor(epsilon), y=ppv, color=factor(method, levels=c("CAMERA", "GSEA-LFC-S", "GSEA-CW-S")))) + geom_boxplot(outlier.size=0.25, stroke=1)
g2 <- g2 + theme_bw()
g2 <- g2 + scale_colour_manual(values=c("#000000", "#E69F00", "#0072B2"))
g2 <- g2 + theme(legend.title=element_blank())
g2 <- g2 + ylab("Positive Predictive Value (%)")
g2 <- g2 + xlab(TeX("Error in $W^{\\perp}$ ($\\alpha$)"))
g2 <- g2 + ylim(50, 100)
